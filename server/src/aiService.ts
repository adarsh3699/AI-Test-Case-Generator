...
export class AIService {
	 ...
	 */
	async generateTestSummaries(files: FileInput[]): Promise<{ summaries: TestSummary[]; aiProvider: string }> {
	 ...
	 */
	private async generateWithGemini(files: FileInput[]): Promise<{ summaries: TestSummary[]; aiProvider: string }> {
...
Generate the test case summaries now:`;
			...
			if (jsonMatch) {
				...
				if (Array.isArray(parsed)) {
					return parsed.filter(
						(item) => item && typeof item.summaryId === "string" && typeof item.summaryText === "string"
					);
				}
			}
	...
	private buildCodePrompt(request: GenerateCodeRequest): string {
		...
		const isReact = fileContent.includes("React") || fileExtension === "tsx" || fileExtension === "jsx";
...
Generate the test code now:`;
	...
	private parseCodeResponse(
		...
		const isTypeScript = fileExtension === "ts" || fileExtension === "tsx";
		const isReact = request.fileContent.includes("React") || fileExtension === "tsx" || fileExtension === "jsx";
		const isPython = fileExtension === "py";
